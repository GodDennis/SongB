(() => {
  ...
  function i(o) {
    ...
    var a = (t[o] = { exports: {} });
    return e[o](a, a.exports, i), a.exports;
  }
  (i.d = (e, t) => {
    for (var o in t)
      i.o(t, o) &&
    ...
    (() => {
      var e = i(471);
      const t = document.querySelectorAll('.nav__link'),
        o = document.querySelector('.wurmap'),
        s = document.querySelector('.passerines'),
        a = document.querySelector('.forest'),
        c = document.querySelector('.singing'),
        n = document.querySelector('.dangerous'),
        d = document.querySelector('.sea'),
        r =
          (document.querySelector('.bird-image'),
          document.querySelector('.audio')),
        p = document.querySelectorAll('.answer-item'),
        l = document.querySelector('.card-body'),
        u = document.querySelector('.card-image'),
        m = document.querySelector('.item-name'),
        g = document.querySelector('.famaly'),
        _ = document.querySelector('.audio-choice'),
        w = document.querySelector('.text'),
        h = document.querySelectorAll('.discrip-item'),
        f = document.querySelectorAll('.playbar'),
        C = document.querySelectorAll('.timebar-circle'),
        v = document.querySelectorAll('.current'),
        k = document.querySelectorAll('.durationplay'),
        y = document.querySelectorAll('.timebar-bar'),
        b = document.querySelector('.bird-image'),
        x = document.querySelector('.name-item'),
        L =
          (document.querySelector('.wasted'),
          document.querySelector('.win'),
          document.querySelector('.count-score'),
      ...
      function j() {
        for (let t = 0; t < p.length; t++)
          (r.src = e.Z[S][X].audio),
            (p[t].textContent = e.Z[S][t].name),
            p[t].addEventListener('click', function () {
              _.classList.remove('play'),
                (_.src = e.Z[S][t].audio),
                p[t].textContent == e.Z[S][t].name &&
                  ((u.src = e.Z[S][t].image),
                  (g.textContent = e.Z[S][t].species),
                  (m.textContent = e.Z[S][t].name),
                  (w.textContent = e.Z[S][t].description),
                  (h[0].style.display = 'none'),
                  (h[1].style.display = 'none'),
                  (l.style.display = 'flex'));
            }),
            p[t].addEventListener('click', () => {
              e.Z[S][X].name == p[t].textContent
                ? ((x.textContent = e.Z[S][X].name),
                  (b.src = e.Z[S][X].image),
                  (p[t].style.backgroundColor = 'green'),
                  setTimeout(() => {
                    p[t].style.backgroundColor = '#303030';
                  }, 1500))
                : ((p[t].style.backgroundColor = 'red'),
                  setTimeout(() => {
                    p[t].style.backgroundColor = '#303030';
                  }, 1500));
            });
      ...
      function A() {
        r.classList.remove('play'), r.pause();
      }
      function q() {
        _.classList.remove('play'), _.pause();
      }
      function E(e, t, i) {
        let o = Math.floor(e.currentTime / 60),
          s = Math.floor(e.currentTime - 60 * o),
          a = Math.floor(e.duration / 60),
          c = Math.floor(e.duration - 60 * a);
        isNaN(e.duration) ||
          (s < 10 && (s = '0' + s),
          c < 10 && (c = '0' + c),
          o < 10 && (o = '0' + o),
      ...
      function O() {
        (x.textContent = '*******'), (b.src = '/img/bird.06a46938.jpg');
      }
      o.addEventListener('click', () => {
        ...
        f[0].addEventListener('click', () => {
          r.classList.contains('play')
            ? A()
            : (r.classList.add('play'), r.play(), q());
        }),
        f[1].addEventListener('click', () => {
          _.classList.contains('play')
            ? q()
            : (_.classList.add('play'), _.play(), A());
        }),
        r.addEventListener('timeupdate', function (e) {
          const { duration: t, currentTime: i } = e.srcElement,
            o = (i / t) * 100;
          (C[0].style.left = `${o}%`), E(r, k[0], v[0]);
        }),
        _.addEventListener('timeupdate', function (e) {
          const { duration: t, currentTime: i } = e.srcElement,
            o = (i / t) * 100;
          (C[1].style.left = `${o}%`), E(_, k[1], v[1]);
        }),
        y[0].addEventListener('click', function (e) {
